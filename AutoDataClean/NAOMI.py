...
def makeBatch(df, feature, window_size):
    ...
    missing_list = df.isnull()[feature]
...
def run_epoch(train, model, exp_data, clip, optimizer=None, batch_size=64, num_missing=None, teacher_forcing=True):
    ...
    while i + batch_size <= exp_data.shape[0]:
        ...
        missing_list = torch.from_numpy(
            np.random.choice(np.arange(1, data.shape[0]), num_missing, replace=False)).long()
    ...
    with trange(pretrain_epochs, file=sys.stdout) as tr:
        for e in tr:
            ...
            optimizer = torch.optim.Adam(
                ...
                lr=lr)
            train_loss = run_epoch(True, model, train_data, clip, optimizer, batch_size=batch_size,
